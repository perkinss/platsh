...
export default function CompetencyWeighting(props) {
    ...
    useEffect(() => {
        fetch('/api/competency_weighting/get_for_user_courses')
            .then(res => res.json())
            .then(
                (result) => {
    ...
    useEffect(() => {
        if (weightings && state.selected_course_id) {
            ...
            let weights = [], names = []
            course.competency_weights.forEach(group => {
                weights.push(group.weight)
                names.push(group.title)
            })
            setValues(weights)
            setLabels(names)
            setCurrentCourseWeighting(course.competency_weights)
        }
    ...
    const handleSaveWeights = () => {
        const courseWeight = { course: state.selected_course_id, weights: currentCourseWeighting.map( (wt) => { return { group_id: wt.id, weight: wt.weight } }) }
        let token = document.querySelector('head>meta[name="csrf-token"]').content
        let body = JSON.stringify(courseWeight)
        fetch('/api/competency_weighting/update',
            {
                method: 'POST',
                body: body,
                headers: {
                        ...
                        <Paper elevation={4}  className={classes.paper}>
                            ...
                            <Grid container spacing={theme.spacing(0)} >
                                <Grid item xs={12} sm={12} md={12}>
                                    <Typography variant={'h5'}>Competencies Weighting</Typography>
                                </Grid>
                            ...
                            <Table size={'small'}>
                                ...
                                { currentCourseWeighting.map( (weighting, index) => {
                                   ...
                                   <TableRow key={`weight-${weighting.id}`}>
                                           <TableCell colSpan={15}>{weighting.title}</TableCell>
